...
interface CloudSyncCfg {
  ...
  /** 0.14.0：宿主额外隐藏名（config.hiddenName）：列举时整个跳过（同 dot 项），不进文件/文件夹列表。 */
...
export function createCloudSync(cfg: CloudSyncCfg): CloudSync {
  ...
  async function pullTail(name: string, n: number): Promise<{ bytes: Uint8Array; item: CloudItem; encrypted: boolean } | null> {
  ...
  //   encrypted：命中的是 encFileName 那份（加密容器）——getHead 据此拒给头片（密文头对 app 无意义，且绝不在库外解）。
  async function pullRange(name: string, offset: number, length: number): Promise<{ bytes: Uint8Array; item: CloudItem; encrypted: boolean } | null> {
    const { item, enc } = await _find(name);
    if (!item) return null;
    const size = item.size || 0;
    const off = Math.max(0, Math.min(offset, size));
    const len = Math.max(0, Math.min(length, size - off));
    if (len === 0) return { bytes: new Uint8Array(0), item, encrypted: enc };
  ...
  async function _walk(subpath: string, out: CloudItem[], depth: number, folders: string[] | null, status: { partial: boolean }): Promise<void> {
    ...
    for (const it of items) {
      ...
      if (isHiddenAny(it.name)) continue;
  ...
  async function listFolder(path: string): Promise<{ files: CloudItem[]; folders: string[]; complete: boolean }> {
    ...
    for (const it of items) {
      if (isHiddenAny(it.name)) continue;   // 隐藏项（.trash/.backup/.<appId>/任意 dot）不进列举——isHidden 深模块
