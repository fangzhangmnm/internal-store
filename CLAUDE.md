...
- **本库 = 家族云同步 store 引擎的户口本体**（2026-08-13 从 WebPaint `src/store/` 立户，种子见 `src/FORK-BASE.md`；此前 MyPWAPatterns `sync-store/` 只是空壳 README）。红线 = MASTER §A（`../docs/MASTER.md`）：无 LWW、处处 If-Match、删除=移到 .trash、冲突必 surface、dirty 永不被驱逐。**改引擎逻辑前 escalate human**（pwa-cloud-store skill）。
